...
def main(message):
    ...
    if msg == '/start':
        ...
        send(id, 'ПРИМЕР РАССЧЁТА ЦЕНЫ: №1) Найти текст, вставить в презентацию, сделать нужный шрифт и его размер, напишу название каждого слайда = 200 руб. Всё это занимает 1,5-3 часа : информацию собираю с 2-4 сайтов, поэтому так долго. №1.1) Если текст надо переписать из 1,5 часовой видеолекции, то по времени это займёт 12,5 часов (1250 руб) и + 30 мин (50 руб) - поиск и вставка схем. = всего 1300 руб. №2) Сокращение текста до сути (без лишней "воды") - занимает не более 1-2 часа. (Не распространяется на №1.1). = 100 - 200 руб соответственно. №3) Проверка и подстройка текста на антиплагиат - занимает обычно от 1 до 4 дней. (Не распространяется на №1.1) = 500 руб (не по таблице). 1 и 2) Основа презентации и правка текста 350руб. Найти текст, вставить в презентацию, сделать нужный шрифт и его размер, напишу название каждого слайда + Сокращение до сути (без лишней "воды"). Занимает 2-4 часа. 2 и 3) Основа презентации и правка текста + проверка на антиплагиат 850руб. Найти текст, вставить в презентацию, сделать нужный шрифт и его размер, напишу название каждого слайда + Сокращение до сути (без лишней "воды") + проверка и под стройка текста на антиплагиат. Занимает обычно от 2 до 4 дней.')
    elif msg == 'Обо Мне':
        send(id, 'Я Андрей делаю презентации на заказ')
    elif msg == 'Заказать Презентацию':
        bot.send_message(message.from_user.id, 'Если вы крутой монтаж, то напишите тему презентации')
